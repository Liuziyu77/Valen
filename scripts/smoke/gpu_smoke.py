...
def main():
    result_path = Path("artifacts/gpu_smoke/result.json")
    ...
    config.update(output="artifacts/gpu_smoke/joint", epochs=4, max_steps=2, tokens_per_step=16384,
    ...
    warmup = dict(config, stage="warmup", data="data/smoke/text.jsonl", output="artifacts/gpu_smoke/warmup", max_steps=1)
    model, _, _ = run(warmup)
    assert all(n.startswith("head.") for n, p in model.named_parameters() if p.requires_grad)
    del model
    torch.cuda.empty_cache()
    text_config = dict(warmup, stage="text", output="artifacts/gpu_smoke/text")
    model, _, _ = run(text_config, initialize="artifacts/gpu_smoke/warmup/latest")
    del model
    torch.cuda.empty_cache()
    joint_config = dict(config, output="artifacts/gpu_smoke/joint_initialized", max_steps=1)
    model, _, _ = run(joint_config, initialize="artifacts/gpu_smoke/text/latest")
    del model
    Path("artifacts/gpu_smoke/predictions.jsonl").write_text("".join(json.dumps(p, ensure_ascii=False) + "\n" for p in predictions), encoding="utf-8")
    ...
    print("VISIONJEV_GPU_SMOKE_PASS", flush=True)
