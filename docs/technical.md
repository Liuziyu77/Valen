...
首页总览图使用简写名称：`Valen-0.8B(SFT)`、`Valen-0.8B(RL)`、`Valen-2B(SFT)`、`Valen-2B(RL)`。`RL` 对应本仓库的 RLCD；General 面板对应下文的 `Valen-Base-*`，Sokoban 面板对应 `Valen-Sokoban-*`。两组实验使用各自任务训练后的 checkpoint，不能视为同一个 checkpoint 的跨任务结果。
