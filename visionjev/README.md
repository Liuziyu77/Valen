...
命令入口位于包顶层，实际逻辑分为数据、模型、训练、评估四部分。下面的路径可直接用于定位实现；端到端流程见[架构说明](../docs/architecture.md)。此处的 `visionjev/evaluation/` 负责通用逐题评估；仓库根目录另有[任务评测包](../evaluation/README.md)，实现 Sokoban 环境和完整游戏评测。
...
`train.py`、`inference.py`、`evaluate.py` 只调用对应子模块的 `main()`。Python 导入使用实现所在的子包，例如 `visionjev.data.compiler.Compiler`。
...
| [test_model_naming.py](../tests/test_model_naming.py) | 模型标识、旧类名与参数键兼容 |
...
模型类使用 `VisualJev`，`VisionJev` 是同一类的兼容别名；响应的 `model` 字段来自包顶层的 `MODEL_NAME`。包名 `visionjev`、命令入口、`visionjev_manifest.json` 和 checkpoint 参数键保持原有命名。
