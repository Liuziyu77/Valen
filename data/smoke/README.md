...
`train.jsonl` 共 6 个 state、15 个问题，其中 13 个有标签。`text.jsonl` 为纯文本阶段使用的子集，包含 4 个 state、9 个问题，其中 7 个有标签。两者不能作为独立的训练/验证划分。测试数据仅检查代码路径，不用于评估模型效果。

推理处理 `train.jsonl` 时返回 6 行响应，包含全部 15 道题；评估生成 13 行逐题预测，`metrics.json` 中记录 `records=6`、`questions=13`。这些计数可用来检查命令是否使用了预期数据。字段约定见[数据格式](../../docs/data-format.md)，输出区别见[推理与评估](../../docs/evaluation.md)。
